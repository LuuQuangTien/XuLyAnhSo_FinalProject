...
class BatchCompleteDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        lbl_title.setStyleSheet("font-size: 22px; font-weight: bold; color: #4CAF50;")
        ...
        if excel_data:
            ...
            lbl_dist.setStyleSheet("font-weight: bold; font-size: 14px; margin-top: 10px;")
            ...
            lbl_dist_data.setStyleSheet("background-color: #222; padding: 10px; border-radius: 5px; color: #ddd; font-size: 13px; line-height: 1.5;")
        ...
        if errors_log:
            ...
            lbl_err.setStyleSheet("font-weight: bold; color: #ff5252;")
            layout.addWidget(lbl_err)
            
            txt_errors = QTextEdit()
            txt_errors.setReadOnly(True)
            txt_errors.setStyleSheet("background-color: #1a1a1a; color: #ff8a80; font-family: Consolas, monospace; border: 1px solid #333;")
            txt_errors.setText("\n".join(errors_log))
        ...
        btn_open_folder.setStyleSheet("padding: 10px; font-weight: bold; background-color: #444; color: white;")
        btn_open_folder.clicked.connect(self._open_folder)
        
        btn_open_excel = QPushButton("📊 Mở File Excel Điểm")
        btn_open_excel.setStyleSheet("padding: 10px; font-weight: bold; background-color: #2196F3; color: white;")
    ...
    def _create_card(self, title, value, bg_color, text_color):
        frame = QFrame()
        frame.setStyleSheet(f"background-color: {bg_color}; border-radius: 8px;")
        lyt = QVBoxLayout(frame)
        lyt.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        lbl_t = QLabel(title)
        lbl_t.setStyleSheet(f"color: {text_color}; font-size: 13px;")
        lbl_t.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        lbl_v = QLabel(value)
        lbl_v.setStyleSheet(f"color: {text_color}; font-size: 24px; font-weight: bold;")
